...
/**
 * Get deals by funnel stage for drill-down
 */
export function getDealsByFunnelStage(stage: 'leads' | 'appointments' | 'negotiations' | 'won') {
  switch (stage) {
    case 'leads':
      return MOCK_DEALS;
    case 'appointments': {
      const activeStatuses: YomiStatus[] = ['受注', 'Aヨミ', 'Bヨミ', 'Cヨミ', 'ネタ'];
      return MOCK_DEALS.filter(d => activeStatuses.includes(d.yomi_status as YomiStatus));
    }
    case 'negotiations': {
      const negotiationStatuses: YomiStatus[] = ['受注', 'Aヨミ', 'Bヨミ'];
      return MOCK_DEALS.filter(d => negotiationStatuses.includes(d.yomi_status as YomiStatus));
    }
    case 'won':
      return MOCK_DEALS.filter(d => d.yomi_status === '受注');
    default:
      return [];
  }
}
