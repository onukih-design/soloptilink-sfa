...
import { INITIAL_FEE_MARGIN_RATE } from '@/lib/constants/margins'
import { useCompanyList } from '@/hooks/use-companies'
import { toast } from 'sonner'
import { Search } from 'lucide-react'

export type OutsourcingOrderFormData = {
  company_id: string
  product: string
  monthly_fee: number
  initial_fee: number
...
interface OutsourcingOrderFormProps {
  ...
  onSubmit: (data: OutsourcingOrderFormData) => void
...
export function OutsourcingOrderForm({
  open,
  onOpenChange,
  onSubmit,
  isSubmitting = false,
}: OutsourcingOrderFormProps) {
  // Form state
  const [companyId, setCompanyId] = useState('')
  const [product, setProduct] = useState('')
  const [monthlyFee, setMonthlyFee] = useState('')
  const [marginRate, setMarginRate] = useState('0.5')
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const formData: OutsourcingOrderFormData = {
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="sm:max-w-[540px] overflow-y-auto">
        <SheetHeader>
          <SheetTitle>営業代行受注登録</SheetTitle>
        </SheetHeader>
